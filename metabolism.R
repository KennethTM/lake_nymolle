...
dic_all_sub <- dic_all |> 
  select(datetime, wtr_dic, ph, dic, anc_predicted)

#Metabolism 2019
metab_data_2019 <- sensor_data_2019 |> 
  left_join(wnd_predict) |> 
  left_join(zmix_2019) |> 
  left_join(dic_all_sub) |> 
  mutate(dosat = o2.at.sat.base(Temp_37),
         ...
         kgas_o2_m_day = k_gas_crusius_vec(wnd_pred_10, Temp_8, "O2"),
         #kgas_co2_m_day = k_gas_ensemble_vec(wnd_pred_10, Temp_8, lake_area, "CO2"),
         kgas_co2_m_day = k_gas_crusius_vec(wnd_pred_10, Temp_8, "CO2"),
...
metab_data_2020 <- bind_rows(sensor_data_2020_1, sensor_data_2020_2, sensor_data_2020_3) |> 
  ...
  mutate(dosat = o2.at.sat.base(Temp_10748214),
         ...
         kgas_o2_m_day = k_gas_crusius_vec(wnd_pred_10, Temp_10748214, "O2"),
         #kgas_co2_m_day = k_gas_ensemble_vec(wnd_pred_10, Temp_10748214, lake_area, "CO2"),
         kgas_co2_m_day = k_gas_crusius_vec(wnd_pred_10, Temp_10748214, "CO2"),
         kgas_o2 = kgas_o2_m_day/24/6,
         kgas_co2_alpha = k_gas_enchance(kgas_co2_m_day, Temp_10748214, ph),
         kgas_co2 = (kgas_co2_m_day*kgas_co2_alpha)/24/6,
         calcification = (c(0, diff(anc_predicted))/2)) |> 
  select(date, depth, datetime, doobs = `DO_mgL_500098`, dosat, lux, wtr = Temp_10748214, zmix, 
         oxygen_dummy, dic_dummy, kgas_o2, kgas_co2, dic, wtr_dic, ph, calcification)
